...
import { ArrowRight, BookOpen, CalendarDays, Heart, MoveRight, Users } from "lucide-react";

const KEGIATAN_PREVIEW = [
    {
        icon: BookOpen,
        title: "Pengajian & Ibadah",
        desc: "Mendukung pengajian rutin dan pelayanan jamaah bersama Takmir Masjid At-Ta'awun.",
        href: "/kegiatan",
    },
    {
        icon: Heart,
        title: "Sosial Masyarakat",
        desc: "Bakti lingkungan, sinoman, dan kolaborasi kepemudaan untuk solidaritas warga.",
        href: "/kegiatan",
    },
    {
        icon: Users,
        title: "Haul Massal",
        desc: "Kegiatan tahunan setiap bulan Sya'ban sebagai wujud penghormatan dan kebersamaan.",
        href: "/haul-massal",
    },
];

const AGENDA = [
    { label: "Kerja Bakti Lingkungan Masjid", date: "5 Jan 2026", kategori: "Sosial" },
    { label: "Pendampingan Pengajian Remaja", date: "15 Jan 2026", kategori: "Ibadah" },
    { label: "Evaluasi Bulanan & Silaturahmi", date: "31 Jan 2026", kategori: "Internal" },
];

const BLOG_PREVIEW = [
    {
        title: "Refleksi Akhir Tahun RISMA Kanggotan",
        excerpt: "Perjalanan setahun penuh, dari kegiatan sosial hingga pembinaan remaja masjid yang terus tumbuh.",
        date: "26 Des 2025",
        href: "/blog",
        img: "https://picsum.photos/seed/risma-refleksi/600/400",
    },
    {
        title: "Kerja Bakti: Wujud Nyata Kepedulian Pemuda",
        excerpt: "Puluhan anggota RISMA turun tangan membersihkan lingkungan masjid dan sekitarnya.",
        date: "6 Jan 2026",
        href: "/blog",
        img: "https://picsum.photos/seed/risma-kerjabakti/600/400",
    },
    {
        title: "Haul Massal: Tradisi yang Menyatukan",
        excerpt: "Ribuan warga berkumpul dalam kegiatan haul massal tahunan, simbol kebersamaan umat.",
        date: "15 Jan 2026",
        href: "/blog",
        img: "https://picsum.photos/seed/risma-haul/600/400",
    },
];
        ...
        <Layout>
                ...
                <div className="w-full grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex gap-10 pt-6 border-t border-border">
                            {[
                                { n: "3+", label: "Tahun Berdiri" },
                                { n: "50+", label: "Anggota Aktif" },
                                { n: "20+", label: "Kegiatan / Tahun" },
                            ].map(({ n, label }) => (
                                <div key={label}>
            ...
            <section className="py-20 border-t border-border">
                ...
                <div className="grid md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border border border-border rounded-xl overflow-hidden">
                    {KEGIATAN_PREVIEW.map(({ icon: Icon, title, desc, href }) => (
                        <Link
                            key={title}
                            href={href}
                ...
                <div className="grid lg:grid-cols-[5fr_7fr] gap-10 lg:gap-16">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col divide-y divide-border border border-border rounded-xl overflow-hidden">
                            {AGENDA.map(({ label, date, kategori }) => (
                                <Link
                                    key={label}
                                    href="/kegiatan"
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">{date} · {kategori}</p>
                                    </div>
                                    <ArrowRight className="size-4 shrink-0 text-muted-foreground group-hover:text-primary transition-colors mt-0.5" />
                                </Link>
                            ))}
                        </div>
                        <Link href="/kegiatan" className="inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline underline-offset-4 mt-5">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-3">
                            {/* Featured */}
                            <Link href={BLOG_PREVIEW[0].href} className="group rounded-xl overflow-hidden border border-border hover:border-foreground/20 transition-colors">
                                <div className="relative h-48 w-full overflow-hidden bg-muted">
                                    <Image
                                        src={BLOG_PREVIEW[0].img}
                                        alt={BLOG_PREVIEW[0].title}
                                        fill
                                        className="object-cover group-hover:scale-[1.03] transition-transform duration-500"
                                    />
                                </div>
                                <div className="p-5">
                                    <p className="text-xs text-muted-foreground mb-2">{BLOG_PREVIEW[0].date}</p>
                                    <h3 className="font-semibold group-hover:text-primary transition-colors leading-snug mb-1">
                                        {BLOG_PREVIEW[0].title}
                                    </h3>
                                    <p className="text-sm text-muted-foreground line-clamp-2">{BLOG_PREVIEW[0].excerpt}</p>
                                </div>
                            </Link>
                            {/* List */}
                            {BLOG_PREVIEW.slice(1).map(({ title, date, href, img }) => (
                                <Link
                                    key={title}
                                    href={href}
                                    className="group flex gap-4 items-center rounded-xl border border-border p-3 hover:border-foreground/20 hover:bg-muted/30 transition-colors"
                                >
